...
プロンプトを書き換えるときは、先に `recipe.json` のコメント
（`_キャラを固定するために` / `_影と床について` / `_タグの数について`）を読んでください。
**新しく足すタグは、Danbooru で現役のもの**（タグAPIで `post_count > 0` かつ
`is_deprecated: false`）だけを使う決まりです（このモデル系列は Danbooru タグで
学習されているため、廃止済み・未登録の語は効きが弱く、ぶれの原因になります）。
採用画像との整合のために**意図的に残した例外**（`silver hair` など）が
`recipe.json` のコメントに理由付きで書いてあるので、それらは消さないでください。
