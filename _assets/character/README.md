...
- `--rembg` を付けると背景除去モデル（birefnet-general-lite）で切り抜きます。
  **床の影・脚の間・髪と肩の間のような囲まれた背景まで抜けます**
  （`python3 -m pip install rembg onnxruntime`。初回だけモデル224MBを取得。
  CPUで1枚あたり50秒程度・メモリを8GB近く使いますが、品質を優先しています。
  u2net は外周に白いモヤ、isnet-anime は囲まれた背景が残るため使いません）
- `--matte` は追加なしで動く簡易版です。単色背景の外周から繋がった部分だけを塗ります
  （服の白や目のハイライトに穴は開きませんが、**影と囲まれた白は残ります**）
- 収まる箱は **幅440px × 高さ660px**。縦横比は保ったまま、この箱に収まる大きさで置かれます
- **全身の絵を置いても、サムネイルには上から42%（胸から上）だけが出ます。**
  全身のまま縮めると顔が10px程度になり、カードでは表情が分からないためです
  （`_render_thumbs.py` の `CHARA_CROP`。元画像は全身のまま残してあるので、
  比率を変えて撮り直すだけで全身にも戻せます）
