...
## 影ごと色で抜く（2026/8/18・ピンク背景への移行）

緑背景での実機1枚目（kurumi/think）で、次の2つが分かった。

1. **negative に shadow を入れても床の影は出ることがある**。影は「暗くなった背景」
   なので色の距離が L1 で100を超え、従来の chroma_cut では不透明のまま残る
2. **モデルは小物を背景色に寄せることがある**。ヘッドホンが黒ではなく緑になり、
   背景との色距離が80前後まで縮んで、色抜きで半透明に食われる状態だった

対処（`chroma_shadow_test.py` が検証。合成画像での確認で、実機の候補では未確認）:

- 背景を **pink background** に変更。キャストの特徴色に高彩度のピンクは無く、
  最近接でも頬の赤み d=104（L1）。緑は kurumi の瞳(green eyes)と衝突していた
- `chroma_cut` に**影の判定**を追加。明るさの比を背景に合わせてから比べ直し、
  向き（最強・最弱チャンネル）と彩度の条件を満たす画素だけを影として抜く。
  黒髪・茶髪・紺髪・金髪・白シャツ・肌・頬・緑の瞳は不透明のまま残ることを確認
- `hi` を 110→100 に（頬の赤み d=104 を不透明側に残すため）、彩度の下限を
  50→90 に（パステルに振れた背景は白シャツ d=75・頬 d=15 まで近づくので、
  色では抜かず birefnet に回す）
- kurumi のヘッドホンをプロンプトで黒に固定、shadow / drop shadow を強調記法で1.4倍

### 実物の候補25枚での追試（2026/8/19）— 色だけで抜くのは撤回

ピンク背景の実物候補で chroma_cut(色抜き)を試したところ、**肌が半透明に食われた**。

- 実際の背景は淡いピンクに振れる: hinata/wave=(248,151,190)、kurumi/think=(246,138,150)
- 暖色の肌とのL1距離が 81〜96 まで縮み、部分透過帯 [40,100) に大量に入る
  （1/9サンプリングで5,000〜8,000画素）。影の残渣も画素単位で残った
- しきい値の調整では「肌を守る」と「影の残渣を消す」が両立しない

対処: **主経路を birefnet に変更**（意味ベースなので肌はキャラだと分かる）。
色は chroma_assist（背景色との距離40以下の完全一致だけを追加で消す。部分透過なし）
として、モデルが取り残す「囲まれた背景」の掃除に降格した。25枚で 6,000〜14,000
画素/枚の取り残しを消せている。chroma_cut は rembg が無い環境の代替として残す。

浮遊する装飾マーク（ハート・矢印・キラキラ。5人中3人に出た）は drop_islands
（最大連結成分の5%未満の島を落とす）で除去。25枚とも目視確認済み。
既知の軽微な残り: 暗い髪の縁にピンクのフチが1px程度残ることがある
（サムネイル縮小でほぼ見えない）。モーション線が手に重なった1枚は薄い点が残った
（negative への motion lines 追加で次回から抑止）。
